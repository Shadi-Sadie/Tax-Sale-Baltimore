...
geocode_census <- function(address, block_lot = NULL) {
    ...
    if (status_code(response) == 200) {
        ...
        if (length(matches) > 0) {
            ...
            return(data.frame(address = address,block_lot = block_lot,  latitude = lat, longitude = lon, GEOID = geoid))
        } else {
            return(data.frame(address = address, block_lot = block_lot, latitude = NA, longitude = NA, GEOID = NA))
        }
    } else {
